...
from get_data_DDI import get_data
...
def df2listss(df):
    ...
    for i in list(zip(df['text'], df['between_sents'], df['label'], df['srcs'], df['trgs'], df['src_types'], df['trg_types'])):
...
def data_pre_process():
    ...
    train_text, train_between_sents, train_label, train_srcs, train_trgs, \
    train_src_types, train_trg_types = df2listss(train_df)

    test_text, test_between_sents, test_label, test_srcs, test_trgs, \
    test_src_types, test_trg_types = df2listss(test_df)
